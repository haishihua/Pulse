...
| **New API** | Pasted key, plus your gateway's address | Any self-hosted [New API](https://github.com/QuantumNous/new-api) relay (a "中转站"): the gateway reports what the account has spent and, where the key carries a quota, a ceiling of its own to draw the ring against. A key issued without one draws no ring until you type a budget that means something to you ([Docs/providers/new-api.md](Docs/providers/new-api.md)) |
